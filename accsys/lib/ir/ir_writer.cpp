...
void AccipitWriter::printFunction(const Function *F) {
    Tracker.incorporateFunction(F); // 给arg和basic block编号，构建map
    Out << "fn ";
    ...
    // ydp modified
    // if (F->getName() == "getint" || F->getName() == "getch" 
    //     || F->getName() == "putint" || F->getName() == "putch"
    //     || F->getName() == "getarray" || F->getName() == "putarray")
    // {
    //     Out << ";\n";
    //     return;
    // }
    ...
    Tracker.incorporateFunction(F); // 重新调用一次
